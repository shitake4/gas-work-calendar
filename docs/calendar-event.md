...
|-----------|-----|------|------|
| `title` | string | ○ | イベントタイトル |
| `startTime` | Date \| string | ○ | 開始日時 |
| `endTime` | Date \| string | ○ | 終了日時 |
| `recurrencePattern` | string | ○ | 繰り返しパターン（下記参照） |
| `interval` | number | - | 繰り返し間隔（デフォルト: 1） |
| `weekdays` | number[] | - | 曜日指定（weekly時） |
| `monthDay` | number | - | 月の日付指定（monthly時） |
| `nthWeekday` | Object | - | 第N曜日指定（monthly時） |
| `until` | Date \| string | △ | 終了日 |
| `count` | number | △ | 繰り返し回数 |
| その他 | - | - | `createCalendarEvent`と同様 |

△ = `until` または `count` のどちらかが必須
...
`weekdays` 配列で曜日を指定（`weekly` パターン時）：
...
const result = createRecurringEvent({
  ...
  until: new Date(2025, 11, 31) // 2025年末まで
});
