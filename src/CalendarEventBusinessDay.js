...
import { createCalendarEventByDate } from './CalendarEventBasic.js';
...
export function createBusinessDayEvent(options) {
  try {
    ...
    // createCalendarEventByDateを使用して予約作成
    const result = createCalendarEventByDate({
      year: targetYear,
      month: targetMonth,
      day: targetDay,
      allDay: isAllDay,
      startTimeStr: options.startTimeStr,
      endTimeStr: options.endTimeStr,
      title: options.title,
